...
const CATEGORY_EMOJIS = {
  ...
  Seafood: "🦐", "Fast Food": "🍔", Soup: "🍲", Salad: "🥙", Any: "🎲",
};
          ...
          <div className="mood-result-wrap">
            {error ? (
              <div className="mood-empty">
                ...
                <p>{error}</p>
                <button className="mood-retry-btn" onClick={() => setSearched(false)}>
                  Reset Filters
                </button>
              </div>
            ) : result ? (
              <div className="mood-result-card">
                ...
                <div
                  ...
                  onClick={() => onRecipeClick && onRecipeClick(result.recipe._id)}
                >
                    ...
                    <div className="result-tags">
                      {result.recipe.category && (
                        <span className="result-tag">
                          {CATEGORY_EMOJIS[result.recipe.category]} {result.recipe.category}
                        </span>
                      )}
                      {result.recipe.weather && result.recipe.weather !== "Any" && (
                        <span className="result-tag">
                          {WEATHERS.find((w) => w.value === result.recipe.weather)?.emoji}{" "}
                          {result.recipe.weather}
                        </span>
                      )}
                      {result.recipe.mealTime && result.recipe.mealTime !== "Any" && (
                        <span className="result-tag">
                          {MEAL_TIMES.find((t) => t.value === result.recipe.mealTime)?.emoji}{" "}
                          {result.recipe.mealTime}
                        </span>
                      )}
                      {result.recipe.time && (
                        <span className="result-tag">⏱ {result.recipe.time}</span>
                      )}
                    </div>
